...
for decoder in ['best_path', 'word_beam_search']:
    for img_filename in Path('../data').files('*.png'):
        ...
        for read_line in read_lines:
           recognized_text = ' '.join(read_word.text for read_word in read_line)
           all_recognized_text.append(recognized_text)  # Accumulate recognized text
...
plt.show()
for recognized_text in all_recognized_text:
